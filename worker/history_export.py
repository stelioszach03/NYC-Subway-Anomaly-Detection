...
SCHEMA = pa.schema([
    (name, dtype) for name, dtype in (
        ("record_type", pa.string()), ("poll_id", pa.int64()), ("observed_ts", pa.int64()),
        ("feed", pa.string()), ("status", pa.string()), ("http_status", pa.int32()),
        ("source_ts", pa.int64()), ("freshness_at_receipt", pa.string()), ("snapshot_sha256", pa.string()),
        ("entity_id", pa.string()), ("is_deleted", pa.bool_()), ("trip_id", pa.string()),
        ("route_id", pa.string()), ("direction_id", pa.int32()), ("start_date", pa.string()),
        ("start_time", pa.string()), ("schedule_relationship", pa.int32()),
        ("stop_schedule_relationship", pa.int32()), ("trip_update_ts", pa.int64()),
        ("stop_id", pa.string()), ("stop_sequence", pa.int32()),
        ("arrival_ts", pa.int64()), ("departure_ts", pa.int64()), ("arrival_delay", pa.int32()),
        ("departure_delay", pa.int32()), ("vehicle_id", pa.string()), ("vehicle_ts", pa.int64()),
        ("latitude", pa.float64()), ("longitude", pa.float64()), ("bearing", pa.float64()),
        ("speed", pa.float64()), ("alert_json", pa.string()),
    )
], metadata={b"mta_history_schema": b"1", b"timestamp_unit": b"unix_seconds_utc"})
...
def normalized_rows(poll: dict, payload: bytes | None):
    base = {"poll_id": poll["id"], "observed_ts": poll["observed_ts"], "feed": poll["feed"],
            "status": poll["status"], "http_status": poll["http_status"], "source_ts": poll["source_ts"],
            "freshness_at_receipt": poll["freshness"], "snapshot_sha256": poll["sha256"]}
    ...
    for entity in feed.entity:
        ...
        if entity.HasField("trip_update"):
            ...
            trip = update.trip
            info = {name: optional(trip, name) for name in ("trip_id", "route_id", "direction_id", "start_date", "start_time", "schedule_relationship")}
            ...
            for stop in update.stop_time_update:
                yield {**common, **info, "record_type": "trip_stop_update", "vehicle_id": vehicle_id,
                       "stop_id": optional(stop, "stop_id"), "stop_sequence": optional(stop, "stop_sequence"),
            ...
            info = {name: optional(vehicle.trip, name) for name in ("trip_id", "route_id", "direction_id", "start_date", "start_time", "schedule_relationship")}
            yield {**common, **info, "record_type": "vehicle", "vehicle_id": optional(vehicle.vehicle, "id"),
                   "vehicle_ts": optional(vehicle, "timestamp"), "stop_id": optional(vehicle, "stop_id"),
            ...
            yield {**common, "record_type": "alert", "alert_json": json.dumps(
...
def enforce_exports(directory: Path, *, now: int, retention_days: int, max_bytes: int):
    days = sorted(p for p in directory.iterdir() if p.is_dir() and not p.is_symlink()
...
def export_day(db: sqlite3.Connection, directory: Path, day: str, *, now: int,
               max_bytes: int, min_free_bytes: int, retention_days: int) -> dict:
    ...
    if target.exists():
        ...
        if (file_sha(target / "observations.parquet") != manifest["sha256"]
    ...
    try:
        ...
        cursor = db.execute("""SELECT p.*, s.payload_gzip FROM polls p
            LEFT JOIN snapshots s ON s.sha256=p.sha256
            WHERE p.observed_ts>=? AND p.observed_ts<? ORDER BY p.observed_ts,p.id""", (start, start + 86400))
        try:
        ...
        manifest = {
            "schema_version": SCHEMA_VERSION, "day_utc": day,
            "generated_at_utc": datetime.fromtimestamp(now, timezone.utc).isoformat(),
            "sha256": file_sha(parquet_path), "bytes": parquet_path.stat().st_size,
            "rows": measured, "polls": poll_count, "row_types": dict(counts), "poll_statuses": dict(statuses),
            "first_poll_ts": first_poll, "last_poll_ts": last_poll,
            "extractor_sha256": file_sha(Path(__file__)), "pyarrow_version": pa.__version__,
...
def export_completed(directory: Path, *, now: int | None = None, retention_days: int = 90,
                     max_bytes: int = 10 * 1024**3, min_free_bytes: int = 2 * 1024**3) -> list[dict]:
    ...
    with (directory / "export.lock").open("a") as lock:
        ...
        with closing(sqlite3.connect((directory / "history.sqlite3").resolve().as_uri() + "?mode=ro", uri=True)) as db:
            ...
            days = [r[0] for r in db.execute("""SELECT DISTINCT strftime('%Y-%m-%d', observed_ts, 'unixepoch')
                FROM polls WHERE observed_ts<? AND observed_ts>=? ORDER BY 1""",
                (now - now % 86400, now - retention_days * 86400))]
            return [export_day(db, output, day, now=now, max_bytes=max_bytes,
                               min_free_bytes=min_free_bytes, retention_days=retention_days) for day in days]
...
def main():
    ...
    print(json.dumps(export_completed(args.state_dir, retention_days=args.retention_days,
                                     max_bytes=args.max_bytes, min_free_bytes=args.min_free_bytes), indent=2))
